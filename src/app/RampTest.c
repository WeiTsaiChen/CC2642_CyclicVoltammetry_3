...
static AD5940Err AppRAMPSeqInitGen(void)
    ...
    dsp_cfg.ADCFilterCfg.BpNotch = bTRUE;                 /* Only used in SINC2 filter */
 ...
 * @return return error code.
*/
static AD5940Err RampDacRegUpdate(uint32_t *pDACData) // NOTE
    {
    ...
    if (AppRAMPCfg.bRampOneDir)
        ...
        switch(AppRAMPCfg.RampState)
            ...
            case RAMP_STATE0: /* Begin of Ramp  */
                AppRAMPCfg.CurrVzeroCode = (uint32_t)((AppRAMPCfg.VzeroStart - 200.0f) / DAC6BITVOLT_1LSB); // Originally, 200.0f
...
static AD5940Err AppRAMPRtiaCal(void)
    {
    fImpPol_Type RtiaCalValue;  /* Calibration result */ // Caution!
...
AD5940Err AppRAMPInit(uint32_t *pBuffer, uint32_t BufferSize)
    ...
    if((AppRAMPCfg.RAMPInited == bFALSE) || \
        ...
        if(AppRAMPCfg.LPTIARtiaSel == LPTIARTIA_OPEN) /* Internal RTIA is opened. User wants to use external RTIA resistor */
            ...
            AppRAMPRtiaCal(); // Calibrate from here
...
static int32_t AppRAMPDataProcess(int32_t *const pData, uint32_t *pDataCount)
    ...
    for(i = 0; i < datacount; i++)
        {
        pData[i] &= 0xffff;	
				temp = -AD5940_ADCCode2Volt(pData[i], AppRAMPCfg.AdcPgaGain, AppRAMPCfg.ADCRefVolt); // Convert the code into voltage from here.	
...
AD5940Err AppRAMPISR(void *pBuff, uint32_t *pCount)
    ...
    *pCount = 0;
    ...
    if(IntFlag & AFEINTSRC_ENDSEQ)
        ...
        AD5940_FIFORd((uint32_t *)pBuff, FifoCnt);
        /* Process data */
        AppRAMPDataProcess((int32_t *)pBuff, &FifoCnt); /* Process the data here */
        *pCount = FifoCnt;
